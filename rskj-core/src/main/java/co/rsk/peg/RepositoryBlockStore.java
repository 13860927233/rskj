...
public class RepositoryBlockStore implements BtcBlockStoreWithCache {
    ...
    public static final int MAX_SIZE_MAP_STORED_BLOCKS = 65535;
    ...
    public StoredBlock getFromCache(Sha256Hash branchBlockHash) throws BlockStoreException {
        ...
        if (branchBlock == null) {
            throw new IllegalArgumentException("Requested block : " + branchBlockHash + " not found in cache ");
        }
        return branchBlock;
    }
    ...
    public BtcBlockInfo getFromCacheMainChain(Sha256Hash branchBlockHash) throws BlockStoreException {
        ...
        if(btcBlockInfoMap == null){
            throw new IllegalArgumentException("Requested block height not found in cache");
        }
        BtcBlockInfo btcBlockInfo = btcBlockInfoMap.get(branchBlockHash);
        if(btcBlockInfo == null){
            throw new IllegalArgumentException("Requested block height and hash not found in cache");
        }
        return btcBlockInfo;
    }
    ...
    public StoredBlock getStoredBlockAtHeight(int height) throws  BlockStoreException{
        ...
        if (height > lastBlockInCache.getHeight()) {
            return null;
        }

        for (int i = 0; i < (lastBlockInCache.getHeight() - height); i++) {
            if (blockHash == null) {
                return null;
            }
